...
async def generate_feedback(query: str, num_questions: int = 3) -> List[str]:
    user_feedback = await llm_provider.generate_object(
        prompt=f"Given the following query from the user, ask some follow up questions to clarify the research direction. The user is trying to reaerch of the given topic and hence return a minimum of {num_questions} questions, but feel free to return more if the original query is not clear: <query>{query}</query>, Output should ONLY be in given JSON format: {format}.. oNCE YOU HAVE YOU HAVE OUTPUT, WAIT AND RECHECK THAT THE OUTPUT IS IN JSON FORMAT OR NOT, IF NOT THEN RE-CREATE THE PROMPT.",
